    ...
    return async (req: NextRequest, next: NextFetchEvent) => {
        ...
        if (requireAuth.includes(pathname)) {
            ...
            if (!token && !authPage.includes(pathname)) {
                ...
                NextResponse.next().headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate')
            ...
            if (token) {
                ...
                if (tokenDecode) {
                    if (typeof tokenDecode !== 'string') {
                        ...
                        if (unixNow > expToken) {
                            return NextResponse.redirect(new URL('/expired/tokenexpired', req.url))
                        }
                    }
                }
            }
