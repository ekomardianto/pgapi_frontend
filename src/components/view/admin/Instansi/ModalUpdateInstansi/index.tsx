...
import { Jasa } from '@/type/Jasa.type'
...
const ModalUpdateInstansi = (props: Proptypes) => {
    const { updateInstansi, setUpdateInstansi, setInstansiData, setToaster, session } = props
    ...
    const handleUpdateInstansi = async (event: FormEvent<HTMLFormElement>) => {
        ...
        console.log(data)
        const result = await instansiServices.updateInstansi(updateInstansi.id, data, session.data?.accessToken);
    ...
    return (
        <Modal onClose={() => setUpdateInstansi({})}>
            ...
            <form onSubmit={handleUpdateInstansi}>
                <Input label='Nama' type='text' name='name' defaultValue={updateInstansi.name} />
                <Input label='Alamat' type='text' name='alamat' defaultValue={updateInstansi.alamat} />
                <Input label='Phone/WA' type='number' name='phone' defaultValue={updateInstansi.phone} />
